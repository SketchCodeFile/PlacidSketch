...
namespace xsketch {
    ...
    struct Report_Slot {
        ...
        Report_Slot() : id(), start_window(0), end_window(0) {}
        Report_Slot(ID_TYPE _id, uint32_t start, uint32_t end) 
            : id(_id), start_window(start), end_window(end) {}
        
        bool operator<(const Report_Slot& other) const {
            uint32_t len1 = end_window - start_window;
            uint32_t len2 = other.end_window - other.start_window;
            if (len1 == len2) {
                return start_window < other.start_window;
            }
            return len1 > len2;
        }
    };
...
public: 
	...
	XStage1(uint32_t memory, int S1Win){
		NumOfWin = S1Win;
		cell_num = memory * 1024 / sizeof(uint32_t) / 3 / NumOfWin;
		for (int i = 0; i < 3; ++i) {
			size[i] = cell_num * 32 / counter_bit[i];
...
public: 
	...
	XStage2(uint32_t memory, double var_input, double error_input, int CellNum, double potential) {
		var_thres = var_input;
	...
	void push(ID_TYPE id, uint32_t win, uint32_t* c) {
		...
		if (win != min_start_window && rand() % (win - min_start_window) == 0) {
			if (win >= min_start_window + P) {
				report_top_k.push_back(Report_Slot<ID_TYPE>(slot[index][min_index].id, slot[index][min_index].start_window, win - 1));
			}
			...
			for (int j = 0; j < bucket_size; ++j) {
				...
				if (slot[i][j].counter[win % P] == 0) {
					if (win >= P + slot[i][j].start_window) {
						...
						report_top_k.push_back(Report_Slot<ID_TYPE>(slot[i][j].id, slot[i][j].start_window, win - 1));
					}
					slot[i][j].clear();
					continue;
				}
...
public: 
	...
	XSketch(uint32_t memory) : win_cnt(0), last_timestamp(0) {
		...
		stage1 = new XStage1<ID_TYPE>(stage1_mem);
		stage2 = new XStage2<ID_TYPE>(stage2_mem);
	}
	XSketch(uint32_t memory, double var_input, double error_input, double ratio_input, int CellNum, int S1Wins, double potential):win_cnt(0),last_timestamp(0){
		...
		double stage1_mem = memory * stage_ratio;
		double stage2_mem = memory * (1 - stage_ratio);
		stage1 = new XStage1<ID_TYPE>(stage1_mem, S1Wins);
		stage2 = new XStage2<ID_TYPE>(stage2_mem, var_input, error_input, CellNum, potential);
	}
	~XSketch() {
		delete stage1;
		delete stage2;
...
private: 
	...
	XStage1<ID_TYPE>* stage1;
	XStage2<ID_TYPE>* stage2;
	double error_thres = error_thres_p;
